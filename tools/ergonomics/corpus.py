#!/usr/bin/env python3
"""Pack and replay the retained INTERACTION.ERGONOMICS editor corpus."""
import argparse
import base64
import gzip
import hashlib
...
def read(path):
    ...
    assert archive["schema"] == 1 and archive["target"] == "editor"
...
def main():
    ...
    if args.mode == "pack":
        ...
        summary = json.loads((args.campaign / "editor" / "summary.json").read_text())
        ...
        archive = {"schema": 1, "target": "editor", "environment": environment,
    ...
    with (args.work / "stdout.txt").open("w") as stdout, (args.work / "stderr.txt").open("w") as stderr:
        result = subprocess.run([str(binary), "editor", str(corpus)], cwd=ROOT,
