//! Revision-bound host analysis, delivered through an independent application source.
    ...
    loop {
        ...
        if app_ready {
            ...
            match command[0] {
                ...
                b'A' => {
                    let old = analysis.as_ref().ok_or("no pending host analysis")?;
                    let result =
                        interaction.complete_at(old.revision(), 0..old.text().len(), "obsolete")?;
                    eprintln!("ANALYSIS {result:?}");
                }
                b'F' => {
                    let fresh = interaction.analysis_snapshot();
                    // One host parse may feed several derivations. No parser is
                    // invoked or retained by REPLAI, and no worker is required.
                    let words: Vec<_> = fresh.text().split_whitespace().collect();
                    let _prefix = words.last();
                    let _host_has_arguments = words.len() > 1;
                    let result = interaction.complete_at(
                        fresh.revision(),
                        0..fresh.text().len(),
                        "ready",
                    )?;
            ...
            match event {
                Event::CompletionRequested => {
                    if interaction.editor().text() == "he" {
                        ...
                        interaction.complete_at(snapshot.revision(), 0..2, "hello")?;
                    }
                    eprintln!("COMPLETION");
                }
